...
void Element::Element_SING()
{
	...
	HeatConduct = 70;
	Description = "Singularity. Creates huge amounts of gravity and negative pressure and destroys everything.";
...
static int update(UPDATE_FUNC_ARGS)
{
	...
	sim->gravmap[(y / CELL) * (XRES / CELL) + (x / CELL)] = parts[i].tmp2++;
...
static void create(ELEMENT_CREATE_FUNC_ARGS)
{
	...
	sim->parts[i].tmp2 = sim->parts[i].life; // Controls gravitational field strength, grows over time
}
